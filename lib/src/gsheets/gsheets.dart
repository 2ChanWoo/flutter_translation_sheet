...
extension MyDateTimeRenderOption on DateTimeRenderOption {
// FORMATTED_VALUE. The default dateTime render option is SERIAL_NUMBER.
  static const _map = <DateTimeRenderOption, String>{
    DateTimeRenderOption.serialNumber: 'SERIAL_NUMBER',
    DateTimeRenderOption.formattedString: 'FORMATTED_VALUE',
  };
...
class Worksheet {
  ...
  Future<List<List<String>>> batchGet(
    List<String> ranges, {
    bool dimenRow = false,
    DateTimeRenderOption dateTime = DateTimeRenderOption.serialNumber,
  }) async {
    var dimension = dimenRow ? dimenRows : dimenColumns;
    var rangesString = ranges.map((e) => 'ranges=$_title!$e').join('&');
    var url =
        '$_sheetsEndpoint$spreadsheetId/values:batchGet?majorDimension=$dimension&valueRenderOption=$renderOption&dateTimeRenderOption=${dateTime.value}&$rangesString'
            .toUri();
  ...
  Future<bool> batchUpdateRows(Map<int, List<String>> rows) async {
    ...
    var url = '$_sheetsEndpoint$spreadsheetId/values:batchUpdate'.toUri();
    final response = await _client.post(
      url,
      body: jsonEncode(
        {
  ...
  Future<bool> batchClearRows(List<int> rows) async {
    ...
    checkResponse(response);
    // print('batchClearRows() response: ${response.body}');
    return response.statusCode == 200;
  }
